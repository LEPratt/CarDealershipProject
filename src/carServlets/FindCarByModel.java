...
@WebServlet("/FindCarByModel")
public class FindCarByModel extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    
    public FindCarByModel() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Car> modelFound = new ArrayList<Car>();		
		String searchModel = request.getParameter("searchModel");		
		
		for (Car find : inventory) 
		{			
			if (find.getModel().equals(searchModel)) 
			{
				modelFound.add(find);
			}
		}
		session.setAttribute("modelFound", modelFound);
	
		RequestDispatcher rs = request.getRequestDispatcher("searchModel.jsp");
